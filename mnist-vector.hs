...
type Image = Vector R
type Label = Vector R
type DataSet = V.Vector (Image, Label)
type W2 = Matrix R
type B2 = Vector R
type W3 = Matrix R
type B3 = Vector R
...
toDataSet = do
  ...
  return $ V.fromList $ zip imgData lblData
...
predict param ds = (V.sum . V.map accept) ds / fromIntegral bsize
...
grad (w2,b2,w3,b3) = V.foldr sumParam acc . V.map getParamDiff
...
main = do
  ...
  -- get dataset
  ds <- V.take trainDataNum <$> toDataSet
